...
from browser_use.browser.navigation_runtime import BrowserSessionNavigationRuntime
from browser_use.browser.profile import BrowserProfile, ProxySettings
from browser_use.browser.watchdog_supervisor import BrowserWatchdogSupervisor
...
class BrowserSession(BaseModel):
	...
	_navigation_runtime: Any = PrivateAttr(default=None)
	_watchdog_supervisor: Any = PrivateAttr(default=None)
	...
	async def reset(self) -> None:
		...
		if self._watchdog_supervisor is not None:
			self._watchdog_supervisor.reset_watchdogs()
		if self._demo_mode:
	...
	def model_post_init(self, __context) -> None:
		...
		self._navigation_runtime = BrowserSessionNavigationRuntime(self)
		self._watchdog_supervisor = BrowserWatchdogSupervisor(self)
	...
	async def on_NavigateToUrlEvent(self, event: NavigateToUrlEvent) -> None:
		...
		await self._navigation_runtime.on_NavigateToUrlEvent(event)
	...
	async def _navigate_and_wait(self, url: str, target_id: str, timeout: float | None = None) -> None:
		...
		"""
		await self._navigation_runtime._navigate_and_wait(url, target_id, timeout=timeout)

	async def on_SwitchTabEvent(self, event: SwitchTabEvent) -> TargetID:
		"""Handle tab switching - core browser functionality."""
		return await self._navigation_runtime.on_SwitchTabEvent(event)
	...
	async def on_TabCreatedEvent(self, event: TabCreatedEvent) -> None:
		...
		await self._navigation_runtime.on_TabCreatedEvent(event)

	async def on_TabClosedEvent(self, event: TabClosedEvent) -> None:
		"""Handle tab closure - update focus if needed."""
		await self._navigation_runtime.on_TabClosedEvent(event)

	async def on_AgentFocusChangedEvent(self, event: AgentFocusChangedEvent) -> None:
		"""Handle agent focus change - update focus and clear cache."""
		await self._navigation_runtime.on_AgentFocusChangedEvent(event)

	async def on_FileDownloadedEvent(self, event: FileDownloadedEvent) -> None:
		"""Track downloaded files during this session."""
		await self._navigation_runtime.on_FileDownloadedEvent(event)
	...
	async def attach_all_watchdogs(self) -> None:
		...
		await self._watchdog_supervisor.attach_all_watchdogs()
